#!/usr/bin/env python3
"""Builds frc-5-reductio.ipynb, the package's Colab notebook: one cell per witnessed predicate of the paper's ledger, each cell addressable
from the ledger page by the predicate's accession key as its stable id (`p05027`: Colab opens the notebook at that cell with `#scrollTo=p05027`).
A predicate cell states the predicate (its text, from the site's ledger JSON) and runs `predicate("5:E1")`: the block script of the
...
md(f"""## Paradoxes of Infinity as Reductio ad Absurdum, [{CITE}]({DOI})
...
INSTALL = "!pip install -q frc-5-reductio --find-links https://finitering.space/pkg/"
...
for r in witnessed:
    lab = f"{PAPER}:{r['label']}"
    code(f"{INSTALL}\n# {lab} ({r['key']}) [{r['tag']}] — the deciding check {dcommon.PREDICATES[lab]}\n{plain(r['predicate'])}\nfrom frc_5_reductio import predicate; predicate(\"{lab}\")", r['key'])

md("""## Summary

The cells above are the paper's python-witnessed predicates; the whole script, check by check, is `python3 reductio.py`
(17 checks, `results.json`), `python3 -m frc_5_reductio` once installed.""", "summary")
code(f"""{INSTALL}
from frc_5_reductio import verify_all
