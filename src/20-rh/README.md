...
Machine checks for *The Riemann Hypothesis over the Holographic Substrate: a Finite-Field Dictionary and the Screen Reading* (Akhtman & Voether, 2026),
paper `20-rh` of the FRC corpus. Seven block scripts, one driver, one notebook; 95 predicate checks (58 EXACT, 34 [approx], 3 [chart]), each keyed to a row of the
...
Discipline: `EXACT` checks are integer-exact finite computations (a pass is a proof on the tested
...
`RH_FAST=1 python3 run_all.py` runs the same 95 predicates at reduced depths (the 10⁸ comb of C2b/C2c and the p = 4801 shell of D4 skipped). Each block script also runs
...
Reproducibility section fixes at commit `b2a3fcec`; `make_notebook.py` writes both). Each block's markdown cell names the paper's
...
The paper carries its predicate ledger as Appendix A (67 rows in blocks A–F, V, Z: 12 imported, 12 declared, 40 theorems, 3 Ω-hard). Every check of this package names the ledger row(s) it witnesses (`LEDGER` in `rhcommon.py`;
...
may be witnessed by several checks (row 20:E12, the shell theorem, by A1–A7, A7b and A9 on six shells) and a check may
