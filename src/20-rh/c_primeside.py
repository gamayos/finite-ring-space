...
Paper-local predicates:
  ...
  C2  Obs. trace      the secular condition Ñ_N(T) = n − ½ recovers the first ten heights: mean error
                      4.4×10⁻⁵, maximum 1.5×10⁻⁴ at N = 10⁶; 3.6×10⁻⁴ at 10³; 6×10⁻⁵ at 10⁵
  C3  Obs. matrix     the colleague matrix of the Chebyshev fit of Ξ(T) = cos(π Ñ_N(T)) on [10, 52]:
                      mean error 5.9×10⁻⁵ at dimension 520, ≈4.5×10⁻⁵ by 620 (the method's floor)
  C4  Obs. jacobi     the comb-built Jacobi matrix: real-symmetric tridiagonal, eigenvalues = the secular
                      roots to 10⁻¹³, the heights to 4.4×10⁻⁵; the ten recurrence coefficients a, b
...
from rhcommon import (check, Comb, count_raw, secular_roots, colleague_roots, jacobi_from_points,
...
def run():
    ...
    check("C2", "secular condition: ten heights to mean 4.4e-5, max 1.5e-4 at N = 10⁶; sharpening with depth", ok,
          "; ".join(f"N={d:.0e}: mean {m:.2e} max {x:.2e}" for d, (m, x) in errs.items()), kind="[approx]")
    roots6 = secular_roots(comb, windows())
    # C4 Jacobi
    ...
    ok = (np.allclose(J, J.T) and np.max(np.abs(np.triu(J, 2))) == 0 and np.max(np.abs(ev - roots6)) < 1e-12
          ...
          and abs(np.mean(np.abs(ev - HEIGHTS[:10])) - 4.4e-5) < 0.6e-5)
    check("C4", "Jacobi matrix: real-symmetric tridiagonal; eig = secular roots to 1e-12; a, b as stated; heights to 4.4e-5", ok,
    ...
    with Timer("colleague matrices"):
        ...
        conv = {d: float(np.mean(nearest_errors(colleague_roots(comb, 10.0, 52.0, d), HEIGHTS[:10]))) for d in degs}
    ok = abs(conv[520] - 5.9e-5) < 1.0e-5 and (FAST or conv[620] < 5.5e-5)
    check("C3", "colleague matrix on [10,52]: mean error 5.9e-5 at dim 520, ≈4.5e-5 by 620", ok,
          "; ".join(f"dim {d}: {v:.2e}" for d, v in conv.items()), kind="[approx]")
    eig520 = colleague_roots(comb, 10.0, 52.0, 520)
    ...
    NB = theta(Tg) / math.pi + 1.0; NC = count_raw(comb, Tg)
    fig, ax = plt.subplots(1, 3, figsize=(13.5, 4.2))
    ax[0].plot(Tg, NC, color="#10325f", lw=1.4, label=r"$\tilde{N}_N(T)=\theta/\pi+1+S_{\rm comb}$")
    ...
    ax[0].set_title(r"C2: secular condition, $\tilde{N}_N = n-\frac{1}{2}$ at the heights", fontsize=10); ax[0].legend(fontsize=8, loc="upper left")
    ...
    ax[1].set_title(f"C3: colleague matrix, dim 520, mean err {conv[520]:.1e}", fontsize=10); ax[1].legend(fontsize=8, loc="lower center")
