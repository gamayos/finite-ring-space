...
`1-algebra-main.ipynb` (Google Colab: one cell per ledger row, any cell on its own, or *Runtime → Run all*, ≈ 45 s) or by `run_all.py`.

Every check names the row of the paper's predicate ledger it witnesses (Appendix A "Predicate ledger and machine verification", 27 rows in blocks A–G, Y,
cited as `1:XN`; public copy `docs/1-algebra/1-algebra-ledger.html`), and the ledger's source column links, for each
machine-verified row, the script at the check that decides it and the Lean module at the row's declaration
...
| `B6` | `b_numbers.py` | EXACT | the Euclidean step count against ⌊log₂ p⌋+1: (55, 34) needs 7 > 6 at p = 59, (987, 610) needs 13 > 10 at p = 1009; F₄ = 3 < 4 | — (a check of the package, no ledger row) |
...
so pip reports it already satisfied once installed; the sdist `frc_1_algebra-<version>.tar.gz` that `src/make_pkg.py`
...
deciding check is `algcommon.ROWS`), that check is printed from the script's own source — the line under its
`# row 1:B2` marker — and every record citing the row is listed with its verdict. The markers in the three block
scripts are the lines the ledger page's source glyph opens (`docs/src/1-algebra/<script>.html#row-<label>`). The rows'
Lean counterparts are the declarations `row_<label>` at the end of `lean/FrcCore/Algebra.lean` (G5's in
`lean/FrcCore/Quaternion.lean`, which imports Algebra) and `lean/FrcLedger/Algebra.lean` (`lean/make_rows.py`), one per
row, with the module as one executable file for the web editor (`lean/web/core/Algebra.lean`, `lean/web/Algebra.lean`).
