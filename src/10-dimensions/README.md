...
`verify_domains.py` is the paper's suite as written (the 200 micro-check names unchanged; two checks added on 22 September 2026 for the round-03 revision, the energy–momentum relation and Buckingham's count; five on 23 September for the electromagnetic domain, row F5), wrapped in a `run()` that
...
Rows B1–B4, C1, C4, D4, E8 are definitions (C6, E6 and F5 composite); A1–A6 imports; D5, D7, E1 realisations with
falsifiers; E7 the Ω-hard Carrier register; no row is open (T1, the electromagnetic domain, closed as F5 on 23 September 2026).
