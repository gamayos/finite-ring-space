...
corpus. Two exact suites, 13 family checks over 212 exact micro-checks, driven by `10-dimensions-main.ipynb` (Google
...
of the paper's predicate ledger it witnesses (the paper's Appendix B "Predicate ledger and machine verification", 41 rows
...
|---|---|---|---|
| `verify_domains.py` | dom.A–H | 202 | A the shell datum and domain algebra on F_13 (33; 10:C2, C3, D2, D6, F2, F4, G1, G3); B the quartet at the unit face in exact rationals (30; 10:E2, E3); C the defining congruences on the lab Carrier (13; 10:E4); D both Carriers, faces, the minimality scan (13, 233) with counterfactuals, temperature and the Unruh closure, the crossing-degree embedding (50; 10:E4, E5, E9, F3, G1); E the covariance witnesses (18; 10:C5); F the window bound, the σ-twisted action, the dualities (15; 10:C5, D1); G realized action, the window ladder, meridian transport, pair canonicity, Buckingham's count (16; 10:C5, D2, G2, G4); H the pair layer and representative inertness (27; 10:E4, E5) |
...
`verify_domains.py` is the paper's suite as written (the 200 micro-check names unchanged; two checks added on 22 September 2026 for the round-03 revision, the energy–momentum relation and Buckingham's count), wrapped in a `run()` that
...
falsifiers; E7 the Ω-hard Carrier register; T1 the open electromagnetic domain (a task, block T).
