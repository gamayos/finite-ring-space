...
predicate ledger (13 September 2026; one script since 24 September 2026). One script, `fourier.py`, five blocks, thirty-nine
...
verification", 51 predicates in blocks A–F, cited as `6:XN`; public copy `docs/6-fourier/index.html`), and the ledger's
...
| C | EXACT | 6/6 | invertibility (Definition `domain`, C1); the 4κ distinct framed bases (Corollary `distinct-domains`, C2); exactly 2κ unordered bases by parity, no further coincidence (Remark `ordered-bases`, C3); S_r(M_m) = M_{m+r} on all pairs (Proposition `meridian-scale`, **00:C2**, C4); the effective step and the (p−1)-periodicity (Corollary `effective-step`, Remark `framed-rational`, C5); the p = 13 ladder and the no-wrap window (Example `zoom-13`, Theorem `zoom`, Remark `two-layers`, C6) |
| D | EXACT | 9/9 | R_s ∈ SO(2, F_p) (Lemma `Rs-rotation`, D1); the isomorphism Z_{4κ} ≅ SO(2, F_p) (Proposition `rotation-isom`, D2); the cardinal values R_0, R_κ, R_{2κ}, R_{3κ} and z_κ = i (Theorem `Weil-equivalence`, D3); the spectral obstruction (Proposition `nogo`, D4); the common character sector (Proposition `charsector`, D5); the cardinal Heisenberg covariance and the expansion of F^[s] in the powers of F (Proposition `heisenberg`, D6); monomiality exactly at the cardinal indices on the 112 intermediate indices of the sweep (D7); cardinal exclusivity on every shell below 200 by the row-0 argument (Theorem `monomial`, D8); the plane as its origin and n+2 free orbits of the rotation group, dim V = n (Corollary `boundary`, D9) |
| E | EXACT / [approx] | 8/8 | Ĝ² = 2n X^κ in the readout algebra, exact modulo Φ_n(X), with both specialisations (Definition `readout`, E1); the entropic cardinal values for every δ_j (Proposition `entropy`, E2); mutually unbiased bases, Maassen–Uffink, the comb log 2 + log 6 = log 12 (E3); the closed form and the two-valued readout (Proposition `closedform`, E4); the p = 13 table and the figure (E5); the δ_1 curve 0.55 vs 0.44 and the odd-projector criterion (Remark `input-dep`, E6); the Galois twist: relabelling whenever ηuj ≡ j, the twist-invariance of the cardinal values, the n = 12 examples (Proposition `twist`, E7); the fractional uncertainty relation along the cycle: the largest overlap c(s) and its bound, mutual unbiasedness exactly at the two Fourier indices, Maassen–Uffink on random states, the p = 13 bound (Proposition `fractional-unc`, E8) |
...
| `B7` | B | EXACT | G = ε(1+i), the two patterns, ε(g⁻¹) = −ε(g), ε(g^u) = (κ/u) ε(g), classes equally populated; 38 frames of p ∈ {5,…,37} and 16 of p = 41; p = 5: (2,0,1,1) at g = 2, (1,1,2,0) at g = 3 | `6:C7`, `6:C10` |
| `B8` | B | EXACT | G G* = −2, G² = 2i, Tr F = iG, Tr F² = 2, Tr F³ = iG*, m_ℓ ≡ ¼ Σ_r i^{−ℓr} Tr F^r (mod p) | `6:C7`, `6:C10` |
...
| `C3` | C | EXACT | F^[s+2κ] = F^[s] J, so B_{s+2κ} = B_s as unordered bases, and B_{s+t} ≠ B_s for t ∉ {0,2κ} (row 0 of F^[t] is (i/2)(1 − g^{−2t}) ≠ 0 off site 0, F^[t] not monomial): 4κ framed domains, exactly 2κ measurement bases | `6:D2`, `6:D7` |
...
| `D8` | D | EXACT | cardinal exclusivity as a theorem (Theorem `monomial`): off j ∈ {−1, 0, 1} row 0 of F^[s] σ F^[s]⁻¹ is x⁻¹ times a quadratic in x = g^j with leading coefficient i(c₀+c₂)c₃(−s) ≠ 0, so at least 4κ−5 nonzero entries for every non-cardinal s; every shell p ≡ 1 (mod 4) below 200 (1920 indices; fewest 11) | `6:E8` |
| `D9` | D | EXACT | the plane as orbits of the cycle (Corollary `boundary`): |SO(2, F_p)| = n = p−1 and it is {R_s}; F_p² is the origin and n+2 free orbits (the n circles x²+y² = c ≠ 0, the two isotropic lines y = ±ix), p² = 1+(n+2)n; R_s is (u, v) ↦ (z_s u, z_s⁻¹ v) on u = x+iy, v = x−iy; dim V = n; the six shells | `6:E10` |
...
| `E7` | E | [approx] | X ↦ ζ^u leaves the cardinal values of every δ_j invariant and relabels the curve of δ_j by s ↦ us whenever ηuj ≡ j (mod n), ηu ≡ 1 (mod 4) — the Born vectors permuted by k ↦ ηuk (Proposition `twist`); n = 12, u = 5: δ_1 0.55 ↦ 0.44 (from every relabelling), δ_3 unchanged at 0.42 | `6:F7` |
| `E8` | E | [approx] | c(s)² ≥ max{1/n, 1 − (n−1)t_s/n} for the largest overlap c(s) between B_0 and B_s; B_s mutually unbiased to B_0 exactly at s ∈ {κ, 3κ}; c(s) = 1 exactly at s ∈ {0, 2κ}; H_{B_0} + H_{B_s} ≥ −2 log c(s) on 40 random states per (shell, s); p = 13: −2 log c(s)/log n = 0, .10, .41, 1, .41, .10 per half-cycle (Proposition `fractional-unc`) | `6:F8` |
