...
GROUPS = [
    ...
    ("The gravitational face: the two faces of the leak and the comparison chart", ["check_o1", "o1_gr_chart"],
     "The two faces of the leak, angular κ/S and temporal κ/(2S), ratio exactly 2 (the double cover, C7); the face-ratio identity 2 ⟺ 2γ − β = 1 over the PPN lattice (C8); the dictionary p_sl = 3(S/κ) r_g and its capacity reading — the open conjecture O4 — in exact rationals; the continuum comparison chart in sympy [approx: continuum chart]: the parametrised isotropic metric's apsidal and clock-deficit coefficients, the ratio deviation (2/3)(2γ − β − 1), Brans–Dicke's −2/(2+ω) with its GR limit."),
...
**Validation Package.** `finite-ring-space/src/{PKG}` — the paper's six in-tree audits (four node scripts, integer-exact;
`o1_gr_chart.py`, sympy over the declared continuum chart; `check_phi.py`, the golden-ratio audit) and the laboratory's
...
with the registry's predicates, which pin the stated totals (58, 10, 10, 11, 7, 10; 84, 28, 25, 25, 10, 20 — the
laboratory's 192) and the lines the paper's rows rest on: the quarter roots ħ = 144, h = 89; the apsidal fraction
