...
| `s13.check_fibrations` | `check_fibrations.py` | EXACT | 30 | the two torus fibrations of the frame variety (A9, X4; master C22) at p = 5, 13, 17: the split fibration by the drive torus C_{p−1} over the unit 2-sphere and the non-split fibration by the boost torus C_{p+1} over the nonsquare-radius 2-sphere, one count p(p²−1); the frame triple G ≅ P¹ × F_p × C_{p−1} |
...
The six suites the laboratory ships (`node run-all.js` there runs them; 208 checks) are run by the registry from
...
| `s13.verify-hopf` | `verify-hopf.js` | 26 | the non-split fibration and the frame counts 2184 = 156 × 14 (A7); the norm-one sphere as SL₂, the adjoint action PGL₂ → SO₃, the two 2-sphere counts, the centralisers and orbits, the bijection G ≅ P¹ × B (A9, X4); Ω-blind at p = 5 |
...
build 189 checks; 192 at phase 6, 208 with the fibration checks of 17 September 2026), the in-tree audits of 1–15 August 2026, `check_s13` at 58 checks since the T30
