#!/usr/bin/env python3
"""Builds frc-13-epi.ipynb, the package's Colab notebook: one cell per witnessed predicate of the paper's ledger, each cell addressable
from the ledger page by the predicate's accession key as its stable id (`p13028`: Colab opens the notebook at that cell with `#scrollTo=p13028`).
A predicate cell states the predicate (its text, from the site's ledger JSON) and runs `predicate("13:F2")`: the script of the
...
import json, re, os
import epi as dcommon

PKG = "13-epi"; NB = f"frc-{PKG}.ipynb"; PAPER = "13"
LEDGER_URL = f"https://finitering.space/{PKG}/"; APPENDIX = "A"          # the paper's ledger page and the appendix that carries the ledger
CITE = "Akhtman, Preprints 2026"; DOI = "https://doi.org/10.20944/preprints202607.1141.v1"                       # the heading's citation, linked to the article
SCRIPT = "epi.py"; SCRIPTS = f"[`{SCRIPT}`](https://finitering.space/src/{PKG}/{SCRIPT[:-3]}.html)"; RUNTIME = "≈ 60 s; the C pass of block kur is the slow part"      # the one script, linked to its source page
...
md(f"""## Finite Field Realisation of the Classical Constants π and e, [{CITE}]({DOI})
...
INSTALL = "!pip install -q frc-13-epi --find-links https://finitering.space/pkg/"
