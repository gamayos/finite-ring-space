#!/usr/bin/env python3
"""Builds 2-geometry-main.ipynb, the package's Colab notebook: one cell per witnessed row of the paper's predicate ledger, each cell addressable
from the ledger page by its stable id (`row-2-D1`: Colab opens the notebook at that cell with `#scrollTo=row-2-D1`).
A row cell states the row (its text, from the site's ledger JSON) and runs `geocommon.row("2:D1")`: the block script of the
...
## Geometry and Constants in Finite Ring Continuum (Akhtman, Symmetry 2026) — the predicate ledger, one cell per row
...
the row, runs the block script of the check that decides it (`a_datum.py`, `b_shell.py`, `c_charts.py`, `d_fourier.py` — each once per session),
...
INSTALL = "!pip install -q frc-2-geometry --find-links https://www.finitering.space/pkg/"
