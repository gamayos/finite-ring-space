...
Each check names the predicate(s) of the paper's ledger it witnesses (LEDGER below; predicates cited as 2:XN),
and the ledger's source column links the check that decides each predicate (the `# predicate 2:XN` marker before it; PREDICATES below).  Where a predicate is proved in Lean
