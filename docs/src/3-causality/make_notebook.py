#!/usr/bin/env python3
"""Builds frc-3-causality.ipynb, the package's Colab notebook: one cell per witnessed predicate of the paper's ledger, each cell addressable
from the ledger page by the predicate's accession key as its stable id (`p03006`: Colab opens the notebook at that cell with `#scrollTo=p03006`).
A predicate cell states the predicate (its text, from the site's ledger JSON) and runs `predicate("3:B2")`: the block script of the
...
import json, re, os
import causality as dcommon

PKG = "3-causality"; NB = f"frc-{PKG}.ipynb"; PAPER = "3"
LEDGER_URL = f"https://finitering.space/{PKG}/"; APPENDIX = "A"          # the paper's ledger page and the appendix that carries the ledger
CITE = "Akhtman, Entropy 2025"; DOI = "https://doi.org/10.3390/e27111098"                       # the heading's citation, linked to the article
SCRIPT = "causality.py"; SCRIPTS = f"[`{SCRIPT}`](https://finitering.space/src/{PKG}/{SCRIPT[:-3]}.html)"; RUNTIME = "≈ 20 s"      # the one script, linked to its source page
...
md(f"""## Euclidean–Lorentzian Dichotomy and Algebraic Causality in Finite Ring Continuum, [{CITE}]({DOI})
...
INSTALL = "!pip install -q frc-3-causality --find-links https://finitering.space/pkg/"
...
for r in witnessed:
    lab = f"{PAPER}:{r['label']}"
    code(f"{INSTALL}\n# {lab} ({r['key']}) [{r['tag']}] — the deciding check {dcommon.PREDICATES[lab]}\n{plain(r['predicate'])}\nfrom frc_3_causality import predicate; predicate(\"{lab}\")", r['key'])

md("""## Summary

The cells above are the paper's python-witnessed predicates; the whole script, check by check, is `python3 causality.py`
(16 checks, `results.json`), `python3 -m frc_3_causality` once installed.""", "summary")
code(f"""{INSTALL}
from frc_3_causality import verify_all
