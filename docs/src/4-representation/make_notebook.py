#!/usr/bin/env python3
"""Builds frc-4-representation.ipynb, the package's Colab notebook: one cell per witnessed predicate of the paper's ledger, each cell addressable
from the ledger page by the predicate's accession key as its stable id (`p04011`: Colab opens the notebook at that cell with `#scrollTo=p04011`).
A predicate cell states the predicate (its text, from the site's ledger JSON) and runs `predicate("4:C1")`: the block script of the
...
import json, re, os
import representation as dcommon
...
md(f"""## Universal Latent Representation in Finite Ring Continuum, [{CITE}]({DOI})
...
INSTALL = "!pip install -q frc-4-representation --find-links https://finitering.space/pkg/"
...
for r in witnessed:
    lab = f"{PAPER}:{r['label']}"
    code(f"{INSTALL}\n# {lab} ({r['key']}) [{r['tag']}] — the deciding check {dcommon.PREDICATES[lab]}\n{plain(r['predicate'])}\nfrom frc_4_representation import predicate; predicate(\"{lab}\")", r['key'])

md("""## Summary

The cells above are the paper's python-witnessed predicates; the whole script, check by check, is `python3 representation.py`
(13 checks, `results.json`), `python3 -m frc_4_representation` once installed.""", "summary")
code(f"""{INSTALL}
from frc_4_representation import verify_all
