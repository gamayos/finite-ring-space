...
realized on `𝔽₁₃` (F2–F4, G3); the window ladder by integer squares (G2). Every declaration is checked to
depend on no axiom (`check_core_axioms.py`).
-/
...
def row_G3 := And.intro @FRC.Dimensions.realized13 (@FRC.Dimensions.recovery13)
-- end ledger rows
